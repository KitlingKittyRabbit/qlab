...
`qlab`是独立的公开基础设施仓库。在标准私有工作区中，它与根仓库、`蓝图`、`qlab_research_private`和`qlab_crypto_data`以兄弟工作树共同存在，但彼此保持独立Git历史。

- 私有研究脚本只能依赖`qlab`正式入口，不得把研究脚本反向包装成共享基础设施。
- crypto正式数据根由相邻的私有数据仓或其他外部目录提供，必须通过`QLAB_CRYPTO_DATA_DIR`显式指定。
- 跨仓库布局、克隆顺序和治理入口见工作区根目录`README.md`；对私有研究执行任何修改前，必须阅读`qlab_research_private/AGENTS.md`。
